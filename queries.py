...
def edit_user(user_id, name, address, bio, avatar_url):
    '''modifies and updates a users' info'''
    try:
    ...
    try:
        ...
        for item in cart:
            ...
            if product.qty > quantity:
                Transaction.create(vendor=vendor, buyer=buyer, product=product, qty=quantity, product_thumb_url=product.thumbnail,
                                   prod_title=product.title, buyer_name=buyer.username, vendor_name=vendor.username)
